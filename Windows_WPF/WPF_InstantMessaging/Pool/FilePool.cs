...
namespace InstantMessaging.Pool
{
    public sealed class FilePool
    {
        ...
        private static readonly ILogger log = Rainbow.LogFactory.CreateLogger<FilePool>();
...
#region PUBLIC METHODS   
        ...
        public Boolean IsThumbnailFileAvailable(String conversationId, String fileDescriptorId, String fileName)
        {
            ...
            else
            {
                ...
                {
                    result = true;
                    log.LogDebug("[IsThumbnailFileAvailable] File already downloaded in Thumbnails part - FileId:[{0}]", fileDescriptorId);
                }
        ...
        public void SetFolderPath(String folderPath)
        {
            try
            {
                ...
                log.LogWarning("[SetFolderPath] FileStorage - folderPath:[{0}] - Files:[{1}] - Thumbnails:[{2}]", folderPath, folderPathFiles, folderPathThumbnails);
            }
            catch (Exception exc)
            {
                log.LogWarning("[SetFolderPath] Impossible to create path to store file thumbnail :\r\n{0}", Util.SerializeException(exc));
            }
        }
...
#region PRIVATE METHODS   
            ...
            do
            {
                if (filesDescriptorToDownload.Count > 0)
                {
                    ...
                    if (DownloadFileDescriptor(fileDescriptorId))
                    {
                        ...
                        log.LogDebug("[BackgroundWorkerFileDescriptotDownload_DoWork] END - SUCCESS - fileDescriptorId:[{0}]", fileDescriptorId);
                    ...
                    else
                    {
                        ...
                        index++;
                        log.LogDebug("[BackgroundWorkerFileDescriptotDownload_DoWork] END - FAILED - fileDescriptorId:[{0}]", fileDescriptorId);
        ...
        private Boolean DownloadFileDescriptor(String fileDescriptorId)
        {
            ...
            fileStorage.GetFileDescriptor(fileDescriptorId, callback =>
            {
                ...
                if (callback.Result.Success)
                {
                    ...
                    log.LogDebug("[DownloadFileDescriptor] fileDescriptor:[{0}]", fileDescriptor.ToString());
                ...
                {
                    log.LogWarning("[DownloadFileDescriptor] Not possible to get file descriptor:[{0}]", Util.SerializeSdkError(callback.Result));
                }
    ...
    #region Thumbnail pool
            ...
            do
            {
                if (thumbnailToDownload.Count > 0)
                {
                    ...
                    if (DownloadThumbnail(fileDescriptorId))
                    {
                        ...
                        log.LogDebug("[BackgroundWorkerThumbnailDownload_DoWork] SUCCESS - fileDescriptorId:[{0}]", fileDescriptorId);
                    ...
                    else
                    {
                        // Download failed - we try for another FileDescriptor
                        index++;
                        log.LogDebug("[BackgroundWorkerThumbnailDownload_DoWork] FAILED - fileDescriptorId:[{0}]", fileDescriptorId);
        ...
        {
            log.LogDebug("[DownloadThumbnailDone] Download done - fileDescriptorId:[{0}]", fileId);
            ...
            {
                log.LogDebug("[DownloadThumbnailDone] Direct Download done - fileDescriptorId:[{0}]", fileId);
                ...
                try
                {
                    using (Stream stream = new MemoryStream(File.ReadAllBytes(filePath)))
                    {
                        ...
                        log.LogDebug("[FileStorage_FileDownloadUpdated] scale thumbnail according density - fileDescriptorId:[{0}] - density:[{1}]", fileId, density);
                    }
                }
                catch
                {
                ...
                log.LogDebug("[FileStorage_FileDownloadUpdated] Thumbnail Download done - fileDescriptorId:[{0}]", fileId);
        ...
        private Boolean DownloadThumbnail(String fileDescriptorId)
        {
            ...
            if (filename == null)
            {
                log.LogWarning("[DownloadThumbnail] Not possible to get filename associated to this thumbnail - fileDescriptorId:[{0}]", fileDescriptorId);
                return false;
            }
            ...
            if (thumbnailDirectToDownload.Contains(fileDescriptorId))
            {
                fileStorage.DownloadFile(fileDescriptorId, this.folderPathFiles, filename, callback =>
                {
                    ...
                    if(!callback.Result.Success)
                        log.LogWarning("[DownloadThumbnail] Not possible to get direct file download:[{0}]", Util.SerializeSdkError(callback.Result));
                    manualEvent.Set();
                });
            }
            else
            {
                fileStorage.DownloadThumbnailFile(fileDescriptorId, this.folderPathThumbnails, filename, callback =>
                {
                    downloadResult = callback.Result.Success;
                    if (!callback.Result.Success)
                        log.LogWarning("[DownloadThumbnail] Not possible to get thumbnail:[{0}]", Util.SerializeSdkError(callback.Result));
                    manualEvent.Set();
                });
            }
