...
if (!RbAvatars.Initialize())
    Util.WriteRed("CANNOT initialize Avatars service ...");
...
void RbAvatars_PeerAvatarUpdated(Rainbow.Model.Peer peer)
{
    ...
    switch(peer.Type)
    {
        case Rainbow.Consts.EntityType.User:
            ...
            Util.WriteBlue($"{CR}New Avatar available for Contact: [{peer.ToString(DetailsLevel.Small)}] - Path:[{path}]");
            break;

        case Rainbow.Consts.EntityType.Bubble:
            path = RbAvatars.GetBubbleAvatarPath(peer.Id);
            Util.WriteBlue($"{CR}New Avatar available for Bubble: [{peer.ToString(DetailsLevel.Small)}] - Path:[{path}]");
            break;

        case Rainbow.Consts.EntityType.Channel:
            path = RbAvatars.GetChannelAvatarPath(peer.Id);
            Util.WriteBlue($"{CR}New Avatar available for Channel: [{peer.ToString(DetailsLevel.Small)}] - Path:[{path}]");
            break;
    }
}
