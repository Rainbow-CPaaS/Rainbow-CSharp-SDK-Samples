...
// ----------------------------------------
// START - Code specific about Avatars service

// Create Avatars service
var RbAvatars = Avatars.GetOrCreateInstance(RbApplication);

// Specify where Avatars will be stored
RbAvatars.SetFolderPathUsedToStoreAvatars(Path.Combine(".", "avatars"));

// Specify which Avatar's kinds we want to manage - here we want Contact, Bubble and Channel
RbAvatars.SetAvatarCategories(Avatars.AvatarCategory.CONTACT, Avatars.AvatarCategory.BUBBLE, Avatars.AvatarCategory.CHANNEL); ;

// Initialize the service - mainly Folders to store each Avatar Category are created
if (!RbAvatars.Initialize())
    ConsoleAbstraction.WriteRed("CANNOT initialize Avatars service ...");

// Indicate that the download can be done
RbAvatars.AllowAvatarDownload(true);

// We want to know when a new Avatar has been updated / downloaded
RbAvatars.PeerAvatarUpdated += RbAvatars_PeerAvatarUpdated;

// END - Code specific about Avatars service
// ----------------------------------------
...
RbBubbles.BubbleAffiliationsPerformed += RbBubbles_BubbleAffiliationsPerformed;
...
void RbAvatars_PeerAvatarUpdated(Rainbow.Model.Peer peer)
{
    ...
    switch(peer.Type)
    {
        case Rainbow.Consts.EntityType.User:
            path = RbAvatars.GetContactAvatarPath(peer.Id);
            ConsoleAbstraction.WriteBlue($"{CR}New Avatar available for Contact: [{peer.ToString(DetailsLevel.Small)}] - Path:[{path}]");
            break;

        case Rainbow.Consts.EntityType.Bubble:
            path = RbAvatars.GetBubbleAvatarPath(peer.Id);
            ConsoleAbstraction.WriteBlue($"{CR}New Avatar available for Bubble: [{peer.ToString(DetailsLevel.Small)}] - Path:[{path}]");
            break;

        case Rainbow.Consts.EntityType.Channel:
            path = RbAvatars.GetChannelAvatarPath(peer.Id);
            ConsoleAbstraction.WriteBlue($"{CR}New Avatar available for Channel: [{peer.ToString(DetailsLevel.Small)}] - Path:[{path}]");
            break;
    }
}
