...
{
    ConsoleAbstraction.WriteRed($"You didn't define an ApiKey in credentials.json file for the first account");
    return;
}
else
    ConsoleAbstraction.WriteRed($"Api Key used: [{credentials.UsersConfig[0].ApiKey}]");
