...
ConsoleAbstraction.WriteBlue($"{CR}Use [S] to send SMS");
...
{
    ConsoleAbstraction.WriteWhite("Enter Phone number: (E.123 international notation format - like \"+33 01 23 45 67 89\" (with \"+33\" for telephone country codes for France, \"+49\" for Germany etc ...)");
    var phoneNumber = ConsoleAbstraction.ReadLine();

    ConsoleAbstraction.WriteWhite("Enter Region: (two uppercase letters as defined in ISO 3166-1 alpha-2 - like \"FR\" for France or \"DE\" for Germany");
    var region = ConsoleAbstraction.ReadLine();

    ConsoleAbstraction.WriteWhite("Enter Message:");
    var message = ConsoleAbstraction.ReadLine();

    ConsoleAbstraction.WriteWhite("With confirmation ? Y/N:");
    var confirmation = ConsoleAbstraction.ReadKey()?.Key == ConsoleKey.Y;

    SdkResult<String> sdkResultString;
    if (confirmation)
    {
        ConsoleAbstraction.WriteGreen($"Trying to send SMS with confirmation ...");
        sdkResultString = await RbSms.SendSmsAndWaitSentConfirmationStatusAsync(phoneNumber, region, message);
    }
    else
    {
        ConsoleAbstraction.WriteGreen($"Trying to send SMS without confirmation ...");
        sdkResultString = await RbSms.SendSmsAsync(phoneNumber, region, message);
    }

    if (sdkResultString.Success)
        ConsoleAbstraction.WriteBlue($"SMS sent status:[{sdkResultString.Data}]");
    else
        ConsoleAbstraction.WriteRed($"Cannot send SMS - Error[{sdkResultString.Result}]");
}
