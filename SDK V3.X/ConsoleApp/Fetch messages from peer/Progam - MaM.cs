...
{
    WriteGreen($"{CR}Use [C] to search a Contact and retrieve messages exchanged with him in Peer to Peer");
...
Contact? SearchForContactsInRoster()
{
    ...
    if(result == null)
        WriteRed($"{CR}No contact found with [{str}]");
    else
        WriteWhite($"Contact found:[{result.ToString(DetailsLevel.Small)}]");
...
Bubble? SearchForBubbles()
{
    ...
    if (result == null)
        WriteRed($"{CR}No bubble found with [{str}]");
    else
        WriteWhite($"bubble found:[{result.ToString(DetailsLevel.Small)}]");
...
void FetchAllMessagesFromPeer(Peer peer)
{
    ...
    Task.Run(async () =>
    {
        try
        {
            ...
            WriteBlue($"Start to fetch all messages from Peer:[{RbApplication.GetPeerDisplayName(peer)}]");
            ...
            do
            {
                ...
                if (mamSdkResult.Success)
                {
                    ...
                    if (mamSdkResult.Data.Count > 0)
                    {
                        ...
                        WriteYellow($"Retrieved: {mamSdkResult.Data.Count} - Time elapsed:[{elapsed.TotalSeconds} sec] - Count:[{messagesCount}]");
                    }
                    else
                        canContinue = false;
                }
                ...
                {
                    WriteRed($"SdkError:{mamSdkResult.Result}");
                    canContinue = false;
                    error = true;
                }
            }
